        ...
        return (
                ...
                <div className='LoginConts'>
                    <form onSubmit={onSubmit}>
                        <input name="email" type="text" className='LoginEmail' placeholder="Email" required value={email} onChange={onChange} /><br/>
                        <input name="password"  className='LoginPassword' type="password" placeholder="Password" required value={password} onChange={onChange} /><br/>
                        {errorMsg && <p className="error">{errorMsg}</p>}
                        <input type="submit" className = 'AuthSubmit' value="회원가입" />
                    </form>
                    <button className='G-SingIn' onClick={onSocialClick} name="google" alt="구글로 로그인">google로 회원가입/로그인</button><br/>
                    <Link to="/routes/SignIn"><button>로그인 하러 가기</button></Link>
                </div>
            </>
             );
        }    
